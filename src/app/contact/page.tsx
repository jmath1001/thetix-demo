...
// One checkable row in the dispatch list
type Candidate = {
  rowId: string;         // slake_session_students.id — sent to the API
  studentName: string;
  sessionDate: string;
  sessionTime: string;
  tutorName: string;
  ...
  reminderSent: boolean; // reminder_sent flag from DB
};
...
export default function ContactCenter() {
  ...
  const fetchLogs = useCallback(async () => {
    setLoadingLogs(true);
    const { data } = await supabase.from('slake_reminder_logs').select('*').order('sent_at', { ascending: false }).limit(200);
    if (data) setLogs(data);
    setLoadingLogs(false);
  }, []);

  // ── Load candidates for selected date ────────────────────────────────────
  // Pulls directly from slake_session_students so we get the reminder_sent flag.
  ...
  const fetchCandidates = useCallback(async (date: string) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        `)
        .eq('slake_sessions.session_date', date)
        .neq('status', 'cancelled');

      if (error) throw error;

      const rows: Candidate[] = (data ?? []).map((r: any) => {
        const sess   = Array.isArray(r.slake_sessions) ? r.slake_sessions[0] : r.slake_sessions;
        const tutor  = Array.isArray(sess?.slake_tutors) ? sess.slake_tutors[0] : sess?.slake_tutors;
        const student = Array.isArray(r.slake_students) ? r.slake_students[0] : r.slake_students;
        return {
          rowId:        r.id,
      ...
      // Auto-select unsent rows that have at least one email
      setSelected(new Set(
        rows.filter(r => !r.reminderSent && (r.studentEmail || r.parentEmail)).map(r => r.rowId)
      ));
    } catch (e: any) {
  ...
  const saveTemplate = async () => {
    ...
    await supabase.from('slake_center_settings')
                  ...
                  return (
                    <div key={c.rowId}
                      ...
                      style={{
                        ...
                        background: c.reminderSent ? '#f9fafb' : 'white',
                        opacity: noEmail ? 0.45 : 1,
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {noEmail && (
                            <span className="text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded-full" style={{ background: '#f3f4f6', color: '#9ca3af' }}>No email</span>
                          )}
                        </div>
                        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                          ...
                          {/* Show which emails will receive */}
                          {(c.studentEmail || c.parentEmail) && !c.reminderSent && (
                            <span className="text-[10px]" style={{ color: '#9ca3af' }}>
                              → {[c.studentEmail, c.parentEmail].filter(Boolean).join(', ')}
                            </span>
                          )}
                        </div>
                      </div>
                ...
                <div>
                  ...
                  {editingTemplate ? (
                    <textarea value={draftBody} onChange={e => setDraftBody(e.target.value)} rows={10}
                      ...
                      style={{ border: '2px solid #e5e7eb', color: '#111827', lineHeight: 1.6 }}
                      onFocus={e => e.currentTarget.style.borderColor = '#6b7280'}
                      onBlur={e => e.currentTarget.style.borderColor = '#e5e7eb'} />
                  ) : (
