...
const CRON_ENABLED = process.env.TUTOR_WEEKLY_CRON_ENABLED === "true";
...
export async function GET(_req: NextRequest) {
  if (!CRON_ENABLED) {
    return NextResponse.json({ skipped: true, reason: "TUTOR_WEEKLY_CRON_ENABLED is not set to true." });
  }
  ...
  const { fromDate, toDate, periodLabel } = getWeekRange();
