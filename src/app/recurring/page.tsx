...
function SessionDot({ status, date, isPast, onClick }: { status: string; date: string; isPast: boolean; onClick?: () => void }) {
  ...
  return (
    <div
      ...
      <div className="w-4 h-4 rounded-full transition-transform group-hover:scale-125"
        style={{ background: color, outline: onClick ? '2px solid transparent' : 'none' }}
        onMouseEnter={e => { if (onClick) (e.currentTarget as HTMLElement).style.outline = `2px solid ${color}`; }}
...
// ── Series card ────────────────────────────────────────────────────────────────
function SeriesCard({
  s, tutors, students, today,
  onEdit, onCancelSeries, onDelete,
  ...
  const loadSessions = async () => {
    ...
    try {
      const data = await fetchSeriesSessions(s.id);
      setSessions(normaliseRows(data));
    } catch {}
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span className="text-[11px] text-[#64748b] flex items-center gap-1">
                <Calendar size={10}/> {DAY_NAMES[s.dayOfWeek]}s
              </span>
              <span className="text-[11px] text-[#64748b] flex items-center gap-1">
              ...
              {sessions.length > 0 && rate !== null && (
                <>
                  ...
                  <span className="text-[10px] font-bold"
                    style={{ color: rate >= 80 ? '#16a34a' : rate >= 60 ? '#f59e0b' : '#dc2626' }}>
                    {rate}% attendance
                  </span>
                  {noShow > 0 && (
                    <span className="text-[10px] text-[#94a3b8]">{noShow} no-show{noShow !== 1 ? 's' : ''}</span>
                  )}
                </>
              )}
            </div>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            {s.status === 'active' && (
              <>
                ...
                <button onClick={() => setConfirmAction('cancel')}
                  className="p-1.5 rounded-lg text-[#94a3b8] transition-colors hover:text-[#dc2626] hover:bg-[#fff5f5]">
                  <X size={14}/>
                </button>
              </>
            )}
            {s.status !== 'active' && (
              <button onClick={() => setConfirmAction('delete')}
                className="p-1.5 rounded-lg text-[#cbd5e1] transition-colors hover:text-[#dc2626] hover:bg-[#fff5f5]">
                <Trash2 size={14}/>
              </button>
            )}
            <button onClick={toggle}
              className="p-1.5 rounded-lg text-[#94a3b8] transition-colors hover:bg-[#f8fafc]">
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs font-bold text-[#dc2626]">
                {confirmAction === 'cancel'
                  ...
                  : `Delete this series record? This cannot be undone.`}
              </p>
            </div>
          ...
          <div className="px-5 py-4">
            {loadingSessions ? (
              <div className="flex items-center gap-2 text-xs text-[#94a3b8]">
                <RefreshCw size={12} className="animate-spin"/> Loading sessions…
              </div>
            ) : sessions.length === 0 ? (
                    ...
                    .map(row => {
                      ...
                      return (
                        <SessionDot
                          ...
                          date={date}
                          isPast={isPast}
                          onClick={canEdit ? () => onEditSession(row, s) : undefined}
                        />
                      );
                    })}
                </div>
                ...
                {future.length > 0 && (
                  <div>
                    ...
                    <div className="space-y-1.5">
                          ...
                          <div key={row.id}
                            ...
                            {canEdit && (
                              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => onEditSession(row, s)}
                                  className="px-2 py-1 rounded-lg text-[10px] font-bold flex items-center gap-1"
                                  style={{ background: '#f1f5f9', color: '#64748b' }}>
                                  <Pencil size={9}/> Edit
                                </button>
                                <button onClick={() => onCancelSession(row, s)}
                                  className="p-1 rounded-lg text-[#cbd5e1] hover:text-[#dc2626]">
                                  <X size={12}/>
                                </button>
                              </div>
                            )}
                      ...
                      {future.length > 4 && (
                        <p className="text-[10px] text-[#94a3b8] text-center pt-1">+{future.length - 4} more upcoming</p>
                      )}
                    </div>
                  </div>
                )}
...
export default function RecurringManager() {
  ...
  const handleDelete = async (id: string) => {
    try { await supabase.from('slake_recurring_series').delete().eq('id', id); await load(); }
    catch (e: any) { alert(e.message); }
  };
  ...
  const commitEdit = async () => {
    ...
    try {
      if (editTab === 'schedule') {
        ...
        const newEnd = endDateFromWeeks(editingSeries.startDate, newTotalWeeks);
        const { error: ue } = await supabase.from('slake_recurring_series').update({ end_date: newEnd, total_weeks: newTotalWeeks }).eq('id', editingSeries.id);
        if (ue) throw ue;
        const sessions = await getSeriesSessions(editingSeries.id);
        const toDrop = sessions.filter(r => { const d = r.slake_sessions?.session_date ?? ''; return d >= today && d > newEnd; }).map(r => r.id);
        if (toDrop.length > 0) { const { error: de } = await supabase.from('slake_session_students').delete().in('id', toDrop); if (de) throw de; }
        if (newTotalWeeks > editingSeries.totalWeeks) {
          ...
          while (toISODate(cursor) <= newEnd) {
            ...
            const { data: ex } = await supabase.from('slake_sessions').select('id, slake_session_students(id)').eq('session_date', isoDate).eq('tutor_id', editingSeries.tutorId).eq('time', editingSeries.time).maybeSingle();
            let sessionId: string;
            if (ex) {
              if (ex.slake_session_students && ex.slake_session_students.length >= 3) throw new Error(`Session is full on ${isoDate}`);
              sessionId = ex.id;
            } else {
              const { data: cr, error: ce } = await supabase.from('slake_sessions').insert({ session_date: isoDate, tutor_id: editingSeries.tutorId, time: editingSeries.time }).select('id').single();
              if (ce) throw ce; sessionId = cr.id;
            }
            const { error: ee } = await supabase.from('slake_session_students').insert({ session_id: sessionId, student_id: student.id, name: student.name, topic: editingSeries.topic, status: 'scheduled', series_id: editingSeries.id });
            if (ee) throw ee;
  ...
  const handleSingleSave = async () => {
    ...
    try {
      const { error: re } = await supabase.from('slake_session_students').update({ status: 'cancelled' }).eq('id', row.id);
      if (re) throw re;
      const { data: ex } = await supabase.from('slake_sessions').select('id, slake_session_students(id)').eq('session_date', newDate).eq('tutor_id', nTutor).eq('time', newTime).maybeSingle();
      let newSessionId: string;
      if (ex) {
        if ((ex.slake_session_students?.length ?? 0) >= 3) throw new Error(`That slot is already full on ${newDate}`);
        newSessionId = ex.id;
      } else {
        const { data: cr, error: ce } = await supabase.from('slake_sessions').insert({ session_date: newDate, tutor_id: nTutor, time: newTime }).select('id').single();
        if (ce) throw ce; newSessionId = cr.id;
      }
      const student = students.find(st => st.id === s.studentId);
      const { error: ie } = await supabase.from('slake_session_students').insert({ session_id: newSessionId, student_id: s.studentId, name: student?.name ?? s.studentName, topic: newTopic, status: 'scheduled', series_id: s.id });
      if (ie) throw ie;
      closeSingleEdit();
  ...
  const handleSingleCancel = async () => {
    ...
    patchSingle({ saving: true, error: null });
    try {
      const { error: e } = await supabase.from('slake_session_students').update({ status: 'cancelled' }).eq('id', singleEdit.row.id);
      if (e) throw e;
      closeSingleEdit();
      ...
      <div className="max-w-4xl mx-auto px-5 pt-5 space-y-4">
          ...
          {([
            ...
            <button key={f.key} onClick={() => setStatusFilter(f.key)}
              ...
              style={statusFilter === f.key
                ? { background: f.activeBg, color: 'white' }
                : { background: f.bg, color: f.color, border: `1.5px solid ${f.color}20` }}>
              {f.label}
              <span className="text-[9px] opacity-70">({counts[f.key]})</span>
            </button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {filtered.map(s => (
              <SeriesCard
                key={s.id} s={s} tutors={tutors} students={students} today={today}
                onEdit={openEdit}
                onCancelSeries={handleCancelSeries}
                onDelete={handleDelete}
                onEditSession={openSingleEdit}
                onCancelSession={openSingleCancel}
              />
            ))}
          </div>
        )}
                ...
                <div className="space-y-4">
                  <div className="px-4 py-4 rounded-xl" style={{ background: '#fef2f2', border: '1px solid #fecaca' }}>
                    ...
                    <p className="text-xs text-[#64748b] leading-relaxed">
                      {editTab === 'duration'
                        ? `Shortening to ${newTotalWeeks} weeks will remove ${sessionsToRemove} scheduled session${sessionsToRemove !== 1 ? 's' : ''}.`
                        : `Moving to ${DAY_NAMES[newDayOfWeek]}s will cancel and recreate ${sessionsToRemove} future session${sessionsToRemove !== 1 ? 's' : ''}.`}
                    </p>
                  </div>
                ...
                <>
                  ...
                  {editTab === 'duration' && (
                    <>
                      ...
                      <div className="flex justify-between px-4 py-3 rounded-xl text-center" style={{ background: '#f8fafc', border: '1px solid #f1f5f9' }}>
            ...
            <div className="flex items-center justify-between px-5 py-4" style={{ background: '#1e293b' }}>
              <div>
                <p className="text-sm font-black text-white">
                  {singleEdit.confirmCancel ? 'Cancel Session?' : 'Edit Session'}
                </p>
                <p className="text-[11px] mt-0.5" style={{ color: 'rgba(255,255,255,0.5)' }}>
                  {singleEdit.series.studentName} · {singleEdit.row.slake_sessions?.session_date}
                  <span className="ml-2 px-1.5 py-0.5 rounded text-[9px] font-bold" style={{ background: 'rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.5)' }}>series intact</span>
                </p>
              </div>
