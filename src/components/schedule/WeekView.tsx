...
import { PlusCircle, Check, X, Loader2, Repeat } from 'lucide-react';
...
export function WeekView({
  activeDates, tutors, sessions, timeOff, students,
  selectedTutorFilter, tutorPaletteMap,
  setSelectedSessionWithNotes, handleGridSlotClick, onInlineBook, refetch,
}: WeekViewProps) {
  ...
  const handleSave = async (key: string, tutor: Tutor, date: string, block: any) => {
    ...
    try {
      await onInlineBook({
        ...
        recurringWeeks: form.recurring ? form.recurringWeeks : 1,
      });
      closeForm(key);
      logEvent('session_booked', {
        studentName: form.student.name, date,
        recurring: form.recurring,
        recurringWeeks: form.recurring ? form.recurringWeeks : 1,
        source: 'inline_week',
      });
      ...
      <div data-inline-form className="flex flex-col gap-2 p-2.5 rounded-xl"
        ...
        <div className="flex items-center gap-2">
          <div className="flex rounded-lg overflow-hidden" style={{ border: '1px solid #e5e7eb' }}>
            <button
              onMouseDown={e => { e.preventDefault(); patchForm(key, { recurring: false }); }}
              className="px-2.5 py-1 text-[9px] font-black uppercase tracking-wider transition-colors"
              style={{ background: !form.recurring ? '#1f2937' : 'white', color: !form.recurring ? 'white' : '#9ca3af' }}>
              Once
            </button>
            <button
              onMouseDown={e => { e.preventDefault(); patchForm(key, { recurring: true }); }}
              className="px-2.5 py-1 text-[9px] font-black uppercase tracking-wider transition-colors flex items-center gap-1"
              style={{ background: form.recurring ? '#6366f1' : 'white', color: form.recurring ? 'white' : '#9ca3af', borderLeft: '1px solid #e5e7eb' }}>
          ...
          {form.recurring && (
            <div className="flex gap-1 ml-auto">
              {[2, 4, 8].map(w => (
                <button key={w}
                  onMouseDown={e => { e.preventDefault(); patchForm(key, { recurringWeeks: w }); }}
                  className="w-6 h-6 rounded text-[9px] font-black transition-colors"
                  style={{
                    background: form.recurringWeeks === w ? '#6366f1' : '#f3f4f6',
                    color: form.recurringWeeks === w ? 'white' : '#6b7280',
                    border: form.recurringWeeks === w ? '1px solid #6366f1' : '1px solid #e5e7eb',
                  }}>
                  {w}w
                </button>
              ))}
        ...
        <button onClick={() => handleSave(key, tutor, date, block)} disabled={!canSave}
          ...
          {form.saving
            ...
            : form.recurring
              ? <><Repeat size={10} /> Book {form.recurringWeeks}w</>
              : 'Book'}
        </button>
      </div>
    );
