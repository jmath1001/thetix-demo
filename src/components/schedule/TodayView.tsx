...
export function TodayView({
  ...
  const handleSave = async (key: string, tutor: Tutor, block: any) => {
    ...
    try {
      const weeksToUse = form.recurringMode === 'until_term_end' && currentTerm ? calculateWeeksUntilTermEnd(todayIso, currentTerm.end_date) : clampWeeks(form.recurringWeeks);
      await onInlineBook({
        tutorId: tutor.id,
        date:    todayIso,
        time:    block.time,
            ...
            <div className="mt-2 space-y-2">
              ...
              {form.recurringMode === 'until_term_end' && currentTerm && (
                <div className="text-[8px] font-semibold rounded px-2 py-1.5" style={{ background: '#dcfce7', border: '1px solid #86efac', color: '#15803d' }}>
                  Until {new Date(currentTerm.end_date + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} ({calculateWeeksUntilTermEnd(todayIso, currentTerm.end_date)} wk{calculateWeeksUntilTermEnd(todayIso, currentTerm.end_date) !== 1 ? 's' : ''})
                </div>
              )}
            </div>
          )}
